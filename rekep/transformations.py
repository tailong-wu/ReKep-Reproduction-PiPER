...
def change_pose_frame_to_cam(pose, frame, degrees=False):
    R_frame = euler_to_rmat(frame[3:6], degrees=degrees) @ np.array([[-1, 0, 0],[0, -1, 0], [0,0,1]])
    R_pose = euler_to_rmat(pose[3:6], degrees=degrees)
    t_frame, t_pose = frame[:3], pose[:3]
    euler_new = rmat_to_euler(R_frame @ R_pose, degrees=degrees)
    t_new = R_frame @ t_pose + t_frame
...
def invert_frame(frame, degrees=False):
    R_frame = euler_to_rmat(frame[3:6], degrees=degrees)
    t_frame = frame[:3]
    euler_new = rmat_to_euler(R_frame.T, degrees=degrees)
    t_new = -R_frame.T @ t_frame
    result = np.concatenate([t_new, euler_new])
    return result